...
const getSuperAdminDashboard = async () => {
  const { rows: [stats] } = await query(`
    SELECT
      (SELECT COUNT(*) FROM partner_profiles p JOIN users u ON u.id = p.user_id WHERE (u.role IS NULL OR u.role IN ('PARTNER', 'TEAM_MEMBER') OR u.role NOT IN ('EMPLOYEE', 'ADMIN', 'HR', 'SUPER_ADMIN'))) as total_partners,
      (SELECT COUNT(*) FROM users u JOIN partner_profiles p ON p.user_id = u.id WHERE u.status = 'active' AND (u.role IS NULL OR u.role IN ('PARTNER', 'TEAM_MEMBER') OR u.role NOT IN ('EMPLOYEE', 'ADMIN', 'HR', 'SUPER_ADMIN'))) as active_partners,
      (SELECT COUNT(*) FROM partner_profiles p JOIN users u ON u.id = p.user_id WHERE p.kyc_status = 'pending' AND (u.role IS NULL OR u.role IN ('PARTNER', 'TEAM_MEMBER') OR u.role NOT IN ('EMPLOYEE', 'ADMIN', 'HR', 'SUPER_ADMIN'))) as pending_kyc,
