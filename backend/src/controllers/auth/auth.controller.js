...
const { sendOtpEmail, sendVerificationEmail, sendEmail } = require('../../services/auth/email.service.js');
const { encrypt } = require('../../utils/crypto');
const { JWT_SECRET, OTP_PEPPER } = require('../../config/jwt.config.js');
const { normalizeIndianMobile, verifyAccessToken, sendSmsOtp } = require('../../services/auth/msg91.service.js');
...
const updatePasswordWithOtp = async (req, res, next) => {
  try {
    ...
    // Validate OTP
    const crypto = require('crypto');
    const { OTP_PEPPER } = require('../../config/jwt.config.js');
    const otpHash = crypto.createHmac('sha256', OTP_PEPPER).update(otp).digest('hex');
