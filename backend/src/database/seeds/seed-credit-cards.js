...
async function seed() {
  ...
  for (const [bankKey, cards] of Object.entries(ALL_CARDS)) {
    ...
    for (const card of cards) {
      ...
      const { rows: existing } = await query('SELECT id FROM products WHERE slug = $1', [cardSlug]);
      if (existing.length > 0) {
        console.log(`  ⏭️  ${card.name} (slug: ${cardSlug}) already exists, skipping.`);
        totalSkipped++;
        continue;
      }
      ...
      `, [
        ...
        JSON.stringify(card.eligibility || {}),
        'fixed',
        0,
        card.min_age || null,
        card.max_age || null,
        ...
        card.short_description || null,
        card.benefits || null,
        card.fees_charges || null,
        card.eligibility_criteria || null,
        card.documents_required || null,
        'Apply Now',
        card.seo_title || null,
        card.seo_description || null,
        card.seo_keywords || null,
        card.priority || 0,
        'Active',
        true,
        true,
  ...
  console.log(`\n════════════════════════════════════════`);
  console.log(`✅ Seed complete: ${totalInserted} inserted, ${totalSkipped} skipped`);
  console.log(`════════════════════════════════════════\n`);
}
