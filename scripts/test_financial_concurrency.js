...
const { query, pool } = require(path.resolve(__dirname, '../backend/src/config/database'));
const { creditWalletFromPayment } = require(path.resolve(__dirname, '../backend/src/modules/wallet/service'));
const { verifyPayment } = require(path.resolve(__dirname, '../backend/src/modules/payment/payment.controller'));
const { handleRazorpayWebhook } = require(path.resolve(__dirname, '../backend/src/modules/wallet/controller'));
...
async function setupTestFixtures() {
  console.log(yellow('\n[SETUP] Creating test database tables & fixtures...'));
  ...
  await query(`UPDATE partner_wallets SET available_balance = 0, total_earned = 0 WHERE partner_id IN ($1, $2)`, [partnerAId, partnerBId]);
...
async function runTests() {
  ...
  // Mock req & res for verifyPayment with order notes belonging to Partner A, but req user is Partner B
  const reqMismatch = {
    body: {
  ...
  // We test order notes mismatch logic inside verifyPayment
  // Mocking Razorpay orders fetch
  const Razorpay = require('razorpay');
  const origFetch = Razorpay.prototype.orders ? Razorpay.prototype.orders.fetch : null;
  ...
  // Execute transaction manually simulating error before COMMIT
  const { getClient } = require(path.resolve(__dirname, '../backend/src/config/database'));
  ...
  try {
    ...
    // Simulate database error / constraint failure
  ...
  // Simulated Razorpay Retry (Same Webhook Payload & Event ID)
