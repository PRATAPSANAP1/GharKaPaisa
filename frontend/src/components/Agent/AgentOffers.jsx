...
// LOANS uses theme colors so must be defined inside the component
const INSURANCE = [
  ...
  { type: "General Insurance", providers: ["Car Insurance", "Bike Insurance", "Travel"],  color: "#F5A623" },
];

function ProductCard({ name, commission, color, badge, C, S }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov ? `0 10px 25px ${C.teal}18` : `0 2px 6px rgba(0,0,0,0.04)`,
    ...
    >
      ...
      <div style={{ fontSize: "11px", color: C.textLight, background: C.bgSecondary, padding: "6px 10px", borderRadius: "8px", marginBottom: "14px" }}>
...
export default function AgentOffers() {
  ...
  // LOANS uses live C values from theme
  const LOANS = [
  ...
  return (
    <div>
        ...
        {tabs.map(t => (
          <button
            ...
            style={{
              ...
              whiteSpace: "nowrap",
              transition: "all 0.2s",
              background: activeTab === t.id ? C.navy : C.card,
              color: activeTab === t.id ? "#fff" : C.textMid,
              boxShadow: activeTab === t.id ? `0 4px 12px rgba(0,0,0,0.15)` : `0 1px 3px rgba(0,0,0,0.06)`,
            }}
          >
            {t.icon}
            {t.label}
      ...
      {activeTab === "credit" && (
        <>
          <SectionTitle title="Premium Credit Cards" sub="Instant digital approval pathways" C={C} />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "14px", marginBottom: "28px" }}>
            {BANKS_CREDIT.map(card => (
              <ProductCard key={card.name} {...card} C={C} S={S} />
            ))}
          </div>

          <SectionTitle title="Co-Branded Cards & Partners" sub="Highly requested shopper cards" C={C} />
      ...
      {activeTab === "loans" && (
        <>
          <SectionTitle title="Instant Loans & Borrowing" sub="Quick verification with minimal documentation" C={C} />
            ...
            {LOANS.map(loan => (
              <div key={loan.type} style={S.card}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "16px" }}>
                  {loan.banks.map(b => (
                    <span key={b} style={{ background: C.bgSecondary, color: C.textMid, borderRadius: "6px", padding: "3px 8px", fontSize: "11px", fontWeight: 600 }}>{b}</span>
                  ))}
                </div>
      ...
      {activeTab === "insurance" && (
        <>
          <SectionTitle title="Insurance Partnerships" sub="Protect customers & receive payouts" C={C} />
