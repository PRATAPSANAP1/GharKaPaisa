...
export default function PartnerVault() {
  ...
  const handleViewOrDownload = async (docId, shouldDownload = false, fallbackUrl = null) => {
    ...
    try {
      ...
      if (res.data?.success && res.data?.data?.url) {
        const targetUrl = res.data.data.url;
        if (shouldDownload) {
          const link = document.createElement('a');
          link.href = targetUrl;
          link.setAttribute('download', '');
          ...
          {filteredDocs.map((doc) => (
            <div key={doc.id} style={{
              ...
              <div style={{
                ...
                <button
                  onClick={() => handleViewOrDownload(doc.id, false, doc.fileUrl)}
                  style={{
                ...
                <button
                  onClick={() => handleViewOrDownload(doc.id, true, doc.fileUrl)}
                  style={{
                    flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px',
