...
export default function SuperAdminReports() {
  ...
  const downloadPartnersCSV = (partners, summary) => {
    ...
    const csvRows = [];
    csvRows.push("PARTNER PERFORMANCE & ROSTER REPORT");
    csvRows.push(`Generated On,${new Date().toISOString().split('T')[0]}`);
    csvRows.push(`Total Registered Partners,${summary.total_partners}`);
    csvRows.push(`Total Active Partners,${summary.total_active_partners}`);
    csvRows.push(`Total Blocked Partners,${summary.total_blocked_partners}`);
    csvRows.push("");
    ...
    const headers = [
      ...
      "Wallet Balance (INR)",
      "Hold Balance (INR)",
      "Total Earned Payouts (INR)",
      "Total Redeemed Amount (INR)",
      "Registration Date"
    ];
    ...
    partners.forEach(p => {
      csvRows.push([
        ...
        p.total_earned || 0,
        p.total_withdrawn || 0,
        `"${p.registration_date ? new Date(p.registration_date).toISOString().split('T')[0] : ''}"`
      ].join(","));
    });
    ...
    link.setAttribute("download", `partners_ledger_report_${new Date().toISOString().split('T')[0]}.csv`);
          ...
          {loadingAnalytics ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "16px" }}>
                ...
                {/* Partner Roster Report Exporter Control Card */}
                <div style={{ ...S.card, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    <h3 style={{ fontSize: "15px", fontWeight: 800, color: C.text, margin: "0 0 6px 0", display: "flex", alignItems: "center", gap: "8px" }}>
                      <Icons.profile size={18} color={C.teal} />
                      Export Partner Roster Report
                    </h3>
                    <p style={{ fontSize: "12px", color: C.textLight, margin: 0, lineHeight: 1.4 }}>
                      Generate a comprehensive real-time CSV spreadsheet of all registered partners including profile info, registration date, wallet balance, redeem metrics, KYC verification status, and active/blocked states.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={handleExportPartners}
                    disabled={loadingExportPartners}
                    style={{
                      background: C.teal,
                      color: "#fff",
                      border: "none",
                  ...
                  >
                    {loadingExportPartners ? "Exporting CSV..." : "Export Partner Report"}
                  </button>
                </div>
              </div>
