...
export default function AdminLogin() {
  ...
  const handleSendOtp = async () => {
    ...
    if (otpAttempts >= 3)
      return setErr(
        "Maximum OTP attempts reached for this session. Please try again later."
      );
    ...
    // ── Mobile OTP via MSG91 SendOTP Web SDK ────────────────────────────────────
    if (isMobile) {
      try {
...
console.log("==============================");
  ...
  const handleSubmit = async (e) => {
    ...
    // FIX: Validate OTP fields BEFORE setting loading, so early returns
    // don't leave the button stuck in "Verifying…" state.
    if (method === "otp") {
    ...
    try {
      if (method === "otp") {
        ...
        if (isMobile) {
          // ── Mobile OTP verification via MSG91 ────────────────────────────────
    ...
    (errResponse) => {
      ...
      reject(
        new Error(errResponse?.message || "Invalid OTP code entered.")
          ...
          // re-derive from profile — simplest: call getMe again (already cached).
      ...
      setLoading((l) => ({ ...l, login: false })); // FIX: always reset on error
    }
  };
          ...
          {showForgot ? (
            ...
            <form onSubmit={handleForgotSubmit}>
              <div style={{ textAlign: "center", marginBottom: "20px" }}>
                <div
                  style={{ fontSize: "20px", fontWeight: 800, color: C.text }}
                >
                  Reset Password
                </div>
                <div
                    ...
                    style={{
                      ...
                      border:
                        method === m
                          ...
                          : `1px solid ${C.border}`,
                      background:
                        method === m ? C.inputBg : "transparent",
                      color: C.text,
                      cursor: "pointer",
                ...
                <div style={{ marginBottom: "20px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <input
                      ...
                      onFocus={(e) => {
                        if (otpSent) e.target.style.border = focusBorder;
                      }}
                      onBlur={(e) =>
                        (e.target.style.border = `1.5px solid ${C.border}`)
                      }
